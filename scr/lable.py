...
outluna_path=r"D:\lungnoduledetection\result/"
...
for fcount, img_file in enumerate(tqdm(file_list)):
    ...
    if mini_df.shape[0] > 0:  # some files may not have a nodule--skipping those
        ...
        for node_idx, cur_row in mini_df.iterrows():
            ...
            # masks = np.ndarray([1,height,width],dtype=np.uint8)#原版

            imgs = np.ndarray([height, width], dtype=np.float32)
            masks = np.ndarray([height, width], dtype=np.uint8)  # 原版
            # masks = np.ndarray([1, height, width], dtype=np.float32)#修改
            center = np.array([node_x, node_y, node_z])  # nodule center
            v_center = np.rint((center - origin) / spacing)  # nodule center in voxel space (still x,y,z ordering)
            # print(np.arange(int(v_center[2])-1,
